...
def main():
    ...
    print(Demon_f"[INFO] 대상 파일 수: {len(target_files)}")
    if not target_files:
    ...
    print(Demon_f"[INFO] 멀티프로세스 workers: {max_workers}")
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in as_completed(futures):
            ...
            if done % 10 == 0 or done == submitted:
                print(Demon_f"[PROGRESS] {done}/{submitted} files parsed...")

    print(Demon_f"[INFO] 총 레코드 수: {len(all_records)}")
    if not all_records:
    ...
    print(Demon_f"[DONE] {SCHEMA_NAME}.{TABLE_NAME} 테이블에 {len(df)}건 적재 완료")
