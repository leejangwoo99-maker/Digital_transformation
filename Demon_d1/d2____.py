...
def insert_to_db(engine, df: pd.DataFrame):
    ...
    with engine.begin() as conn:
        for st, tbl in TABLE_MAP.items():
            ...
            print(Demon_f"[DB] Inserted {len(sub)} rows into {SCHEMA_NAME}.{tbl}")
...
def main():
    ...
    print(Demon_f"[INFO] Total files found = {len(files)}")
    ...
    print(Demon_f"[INFO] Using multiprocessing: {nproc} processes")
    ...
    with Pool(processes=nproc) as pool:
        for rows in pool.imap_unordered(worker_parse_one, files, chunksize=10):
            ...
            if parsed_files % 20 == 0 or parsed_files == len(files):
                print(Demon_f"[PROGRESS] files={parsed_files}/{len(files)}, total_rows={len(all_records)}")

    print(Demon_f"[INFO] Total parsed rows = {len(all_records)}")
