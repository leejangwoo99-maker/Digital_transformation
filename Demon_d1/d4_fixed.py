...
def log_boot(msg: str) -> None:
    print(Demon_f"[BOOT] {msg}", flush=True)

def log_info(msg: str) -> None:
    print(Demon_f"[INFO] {msg}", flush=True)

def log_retry(msg: str) -> None:
    print(Demon_f"[RETRY] {msg}", flush=True)

def log_error(msg: str) -> None:
    print(Demon_f"[ERROR] {msg}", flush=True)
    ...
    while True:
        try:
            ...
            with _ENGINE.connect() as conn:
                conn.execute(text("SET work_mem TO :wm"), {"wm": WORK_MEM})
                conn.execute(text("SELECT 1"))
            log_info(Demon_f"DB connected | {_masked_db()} | pool_size=1 max_overflow=0 work_mem={WORK_MEM}")
            return _ENGINE
        except Exception as e:
...
def run_backfill_for_window(engine, ws: datetime, we: datetime, states: Dict[str, StationState]) -> None:
    ...
    for table, station in SRC_TABLES:
        ...
        pcount = len(pairs)
        total_pairs += pcount
        log_info(Demon_f"[BACKFILL][PAIR ] {station} pairs={pcount} (pending_ng={'Y' if states[station].pending_ng_ts else 'N'}, in_manual={'Y' if states[station].in_manual else 'N'})")

        ins = insert_rows(engine, pairs) if pcount > 0 else 0
        total_ins += ins
        log_info(Demon_f"[BACKFILL][INS  ] {station} attempted_insert={ins} (dedup by PK/UNIQUE)")
...
# =========================
def main():
    log_boot(Demon_f"backend afa_fail_wasted_time FIXED starting | prod_day={FIXED_PROD_DAY} shift={FIXED_SHIFT} (MP=1, loop=5s, warm-start backfill=ON)")
    ...
    log_info(Demon_f"FIXED window = {FIXED_WS:%Y-%m-%d %H:%M:%S} ~ {FIXED_WE:%Y-%m-%d %H:%M:%S} (KST)")
    ...
    while True:
        ...
        try:
            ...
            last_pk_str = ", ".join([Demon_f"{k}={(v.strftime('%Y-%m-%d %H:%M:%S') if v else 'None')}" for k, v in last_to_ts.items()])
            log_info(Demon_f"[LAST_PK] shift={FIXED_SHIFT} prod_day={FIXED_PROD_DAY} window={FIXED_WS:%Y-%m-%d %H:%M:%S}~{FIXED_WE:%Y-%m-%d %H:%M:%S} | {last_pk_str}")
            ...
            for table, station in SRC_TABLES:
                ...
                if cursor_base > FIXED_WE:
                    ...
                    log_info(Demon_f"[FETCH] {station} cursor>{FIXED_WE:%H:%M:%S} -> skip")
                    continue
                ...
                pcount = len(pairs)
                total_pairs += pcount
                log_info(Demon_f"[PAIR ] {station} pairs={pcount} (pending_ng={'Y' if states[station].pending_ng_ts else 'N'}, in_manual={'Y' if states[station].in_manual else 'N'})")
            ...
            log_info(Demon_f"[LOOP ] FIXED {FIXED_PROD_DAY} {FIXED_SHIFT} | fetched={total_fetched} | pairs={total_pairs} | attempted_insert={total_attempted}")
