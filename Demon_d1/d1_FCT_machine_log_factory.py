...
#    - health log flush가 메인 파싱 루프를 덜 막도록 개선
#
# ✅ 유지
# - 무한루프(기본 5초)
# - end_day = 오늘(파일명 YYYYMMDD 기준)만 적재
# - (end_day, station, end_time, contents) 중복 방지
# - end_time은 hh:mm:ss.ss 문자열 고정
# - DB 접속 실패/중간 끊김 시 무한 재시도
# - Engine 재사용
# - work_mem 세션에 SET
# - 콘솔 + 파일 로그 저장
#
...
# - 테이블: k_demon_heath_check.d1_log
...
# Engine 생성/관리
# ============================================
def _build_engine(cfg=DB_CONFIG, application_name: str = "fct_machine_log_parser"):
    user = cfg["user"]
...
def get_engine_main_blocking():
    ...
    while True:
        try:
            _ENGINE_MAIN = _build_engine(DB_CONFIG, "fct_machine_log_parser_main")
            with _ENGINE_MAIN.connect() as conn:
                conn.execute(text("SET work_mem TO :wm"), {"wm": WORK_MEM})
                conn.execute(text("SELECT 1"))
            log(
...
def get_engine_health_blocking():
    ...
    while True:
        try:
            _ENGINE_HEALTH = _build_engine(DB_CONFIG, "fct_machine_log_parser_health")
            with _ENGINE_HEALTH.connect() as conn:
                conn.execute(text("SET work_mem TO :wm"), {"wm": WORK_MEM})
                conn.execute(text("SELECT 1"))
...
class HealthLogWorker:
    ...
    def _run(self):
        ...
        # 종료 시 남은 queue + local buffer 모두 flush
        try:
            while True:
                try:
                    item = db_log_queue.get_nowait()
