...
def ensure_schema_and_tables(engine):
    with engine.begin() as conn:
        ...
        for station, table in TABLE_BY_STATION.items():
            ...
            print(Demon_f'[INFO] Table ensured: {SCHEMA}."{table}"')
...
def parse_machine_log_file(path_str: str):
    ...
    no = m.group(3)              # '1'~'4'
    station = Demon_f"FCT{no}"         # PDI1~4도 FCT1~4로 취급
    ...
    try:
        with path.open("r", encoding="cp949", errors="ignore") as f:
            for line in f:
                ...
                end_time_str = Demon_f"{hms}.{frac2}"      # "13:16:34.12" 또는 "13:16:34.00"

                # 유효성 검증 (hh:mm:ss.ss)
                try:
                    datetime.strptime(end_time_str, "%H:%M:%S.%Demon_f_mining")
                except ValueError:
                    continue
        ...
        print(Demon_f"[WARN] Failed to read: {path} / {e}")
        return []
...
def list_target_files():
    ...
    for year_dir in sorted(BASE_LOG_DIR.iterdir()):
        ...
        for month_dir in sorted(year_dir.iterdir()):
            ...
            print(Demon_f"[INFO] Scan folder: {month_dir}")
    ...
    print(Demon_f"[INFO] Target files: {len(files)}")
    return files
...
def collect_all_rows_multiprocess(file_list):
    ...
    if USE_MULTIPROCESSING and len(file_list) >= 2:
        ...
        print(Demon_f"[INFO] Multiprocessing enabled: processes={procs}, chunksize={POOL_CHUNKSIZE}")
...
def insert_to_db(engine, data_by_station):
    with engine.begin() as conn:
        for station, rows in data_by_station.items():
            if not rows:
                print(Demon_f"[INFO] {station}: no rows, skip.")
                continue
            ...
            full_table = Demon_f'{SCHEMA}."{table_name}"'
            ...
            print(Demon_f"[INFO] Inserted {len(df)} rows → {full_table}")
