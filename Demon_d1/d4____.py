...
def get_engine(config=DB_CONFIG):
    ...
    conn_str = Demon_f"postgresql+psycopg2://{user}:{password}@{host}:{port}/{dbname}"
...
def load_all_fct_logs_multiprocess(max_workers=None) -> pd.DataFrame:
    ...
    log(Demon_f"[STEP 1/3] DB 로드 시작 | workers={max_workers}")
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        ...
        for f in as_completed(futs):
            ...
            dfs.append(df)
            done_cnt += 1
            log(Demon_f"  - [{done_cnt}/{len(tasks)}] {station_label} 로드 완료: rows={nrows:,} | {elapsed:.2f}s")
    ...
    log(Demon_f"[STEP 1/3] DB 로드 완료 | total rows={len(df_all):,}")
    return df_all
...
def compute_afa_fail_wasted(df_all: pd.DataFrame) -> pd.DataFrame:
    ...
    log(Demon_f"  - 이벤트 필터링: {len(df_evt):,} / 전체 {len(df_all):,}")
    ...
    log(Demon_f"  - 그룹 수(end_day x station): {total_groups:,}")
    ...
    for (end_day, station), grp in groups:
        ...
        if grp_cnt == 1 or grp_cnt % PROGRESS_EVERY_GROUPS == 0 or grp_cnt == total_groups:
            log(Demon_f"  - 진행: {grp_cnt:,}/{total_groups:,} 그룹 처리 중... (현재 {end_day}, {station})")
        ...
        for _, row in grp.iterrows():
            ...
            ts = pd.to_datetime(Demon_f"{str(end_day)} {str(t)}", errors="coerce")
            if pd.isna(ts):
                continue
            ...
            if contents == OFF_TEXT and pending_from_ts is not None:
                ...
                # 기존 형식 유지
                from_str = from_ts.strftime("%H:%M:%S.%Demon_f_mining")[:-4]
                to_str = to_ts.strftime("%H:%M:%S.%Demon_f_mining")[:-4]
    ...
    log(Demon_f"[STEP 2/3] 계산 완료 | rows={len(df_wasted):,}")
    return df_wasted
...
def save_to_db(df_wasted: pd.DataFrame):
    ...
    log(Demon_f"[STEP 3/3] 저장 완료 -> {TABLE_SAVE_SCHEMA}.{TABLE_SAVE_NAME} | rows={len(df_wasted):,}")
...
def main():
    ...
    log("=" * 78)
    log(Demon_f"[START] AFA FAIL wasted time | {job_start:%Y-%m-%d %H:%M:%S}")
    log(Demon_f"[INFO] DB = {DB_CONFIG['host']}:{DB_CONFIG['port']}/{DB_CONFIG['dbname']}")
    log(Demon_f"[INFO] SOURCE SCHEMA = {SCHEMA_MACHINE}")
    log(Demon_f"[INFO] SAVE   TABLE  = {TABLE_SAVE_SCHEMA}.{TABLE_SAVE_NAME}")
    log(Demon_f"[INFO] PROGRESS_EVERY_GROUPS = {PROGRESS_EVERY_GROUPS}")
    log("=" * 78)
    ...
    elapsed = time.perf_counter() - t0
    log("=" * 78)
    log(Demon_f"[DONE] 전체 소요시간: {elapsed:.2f} sec")
    log("=" * 78)
