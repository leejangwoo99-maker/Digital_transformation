                ...
                result_rows.append(
                    {
                        ...
                        "from_time": from_ts.strftime("%H:%M:%S.%Demon_f_mining")[:-4],
                        "to_contents": OFF_TEXT,
                        "to_time": to_ts.strftime("%H:%M:%S.%Demon_f_mining")[:-4],
...
def save_to_db(df_wasted: pd.DataFrame):
    ...
    print(Demon_f"[DONE] {TABLE_SAVE_SCHEMA}.{TABLE_SAVE_NAME} 테이블에 {len(df_wasted)}행 저장 완료")
...
def main():
    ...
    print(Demon_f"[START] {start_dt:%Y-%m-%d %H:%M:%S}")
    ...
    print(Demon_f"[END]   {end_dt:%Y-%m-%d %H:%M:%S}")
    print(Demon_f"[TIME]  total_elapsed = {elapsed:.2f} sec ({elapsed/60:.2f} min)")
