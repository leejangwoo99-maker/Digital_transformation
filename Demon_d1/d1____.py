...
def ensure_schema_and_tables(engine):
    with engine.begin() as conn:
        ...
        for st, tbl in TABLE_BY_STATION.items():
            ...
            print(Demon_f'[INFO] Table ensured: {SCHEMA}."{tbl}"')
...
def parse_machine_log_file(path_str: str):
    ...
    no = m.group(3)              # '1'~'4'
    station = Demon_f"FCT{no}"         # PDI도 FCT로 취급
    ...
    try:
        with path.open("r", encoding="cp949", errors="ignore") as f:
            for line in f:
                ...
                end_time_str = Demon_f"{hms}.{frac2}"  # "13:16:34.12" or "13:16:34.00"
