...
def get_engine(cfg):
    ...
    conn_str = Demon_f"postgresql+psycopg2://{cfg['user']}:{pw}@{cfg['host']}:{cfg['port']}/{cfg['dbname']}"
...
def bootstrap_fct_database(engine_):
    ...
    print(Demon_f"[OK] bootstrap done: {OUT_SCHEMA}.{OUT_TABLE} ensured (NO DROP)")
...
def apply_file_path_skip(df0: pd.DataFrame, processed_paths: set) -> pd.DataFrame:
    ...
    print(Demon_f"[OK] file_path skip applied: skipped_rows={skipped_rows} / remain_rows={len(out)} / unique_file_path_skipped={skipped_files}")
    return out
...
def load_source(engine_, d_from: date, d_to: date) -> pd.DataFrame:
    ...
    """
    with engine_.connect() as conn:
        df = pd.read_sql(text(sql), conn, params={"Demon_d1": d_from, "d2": d_to})

    if df.empty:
        print(Demon_f"[SKIP] source 0 rows in range {d_from} ~ {d_to}")
        return df
...
def build_steps_and_setup_ct(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(Demon_f"[OK] okng count check: bad_PD={len(bad_pd)} bad_NonPD={len(bad_np)}")

    before = len(work)
    work = work[~work["step_description"].isin(DROP_STEPS)].copy()
    after = len(work)
    if before != after:
        print(Demon_f"[OK] dropped steps rows: {before - after}")
...
def snap_group_cycle_to_fct(df: pd.DataFrame, fct: pd.DataFrame) -> pd.DataFrame:
    ...
    print(Demon_f"[OK] snap group cycle: matched_groups={matched_groups}/{total_groups} (rows with fct_end_sec={matched_rows})")
...
def _copy_one_chunk(engine_, staging: str, chunk: pd.DataFrame, start: int, end: int):
    ...
    try:
        ...
        print(Demon_f"[OK] bulk upsert chunk: {start}~{end-1} ({end-start} rows)")
...
def bulk_upsert_copy(engine_, df: pd.DataFrame):
    ...
    while start < total:
        ...
        if dropped > 0:
            print(Demon_f"[WARN] staging chunk PK duplicates dropped: {dropped} (rows {before_rows} -> {len(chunk)})")
        ...
        while True:
            try:
                ...
                attempt += 1
                msg = str(e)
                print(Demon_f"[ERR] COPY+UPSERT failed (attempt={attempt}/{COPY_RETRY_PER_CHUNK}) at chunk {start}~{end-1}")
                print("      ", msg[:300])
                ...
                if attempt >= COPY_RETRY_PER_CHUNK:
                    ...
                    if next_sizes:
                        ...
                        print(Demon_f"[DOWN] COPY_CHUNK_ROWS downshift -> {current_chunk_rows} (retry chunk from {start})")
                        # end 재계산 후 chunk 재구성
                        end = min(start + current_chunk_rows, total)
                        chunk = df_db.iloc[start:end].copy()
                        before_rows = len(chunk)
    ...
    print(Demon_f"[DONE] UPSERT(COPY) 완료: {total} rows / sec={time.time()-t0:.2f}")
...
def direct_upsert(engine_, df: pd.DataFrame):
    ...
    if dropped > 0:
        print(Demon_f"[WARN] direct upsert PK duplicates dropped: {dropped} (rows {before} -> {len(df2)})")
    ...
    try:
        ...
        with raw.cursor() as cur:
            ...
            for start in range(0, total, DIRECT_UPSERT_BATCH_ROWS):
                ...
                raw.commit()
                print(Demon_f"[OK] direct upsert batch: {start}~{end-1} ({end-start} rows)")
    ...
    print(Demon_f"[DONE] UPSERT(DIRECT) 완료: {len(df2)} rows / sec={time.time()-t0:.2f}")
...
def upsert_auto(engine_, df: pd.DataFrame):
    ...
    if n <= UPSERT_DIRECT_THRESHOLD_ROWS:
        print(Demon_f"[MODE] DIRECT UPSERT (rows={n} <= threshold={UPSERT_DIRECT_THRESHOLD_ROWS})")
        direct_upsert(engine_, df)
    else:
        print(Demon_f"[MODE] COPY+STAGING UPSERT (rows={n} > threshold={UPSERT_DIRECT_THRESHOLD_ROWS})")
...
def main():
    ...
    print("=" * 120)
    print(Demon_f"[RUN] START : {run_start.strftime('%Y-%m-%d %H:%M:%S')}")
    print(Demon_f"[RUN] MODE  : DATE_MODE={DATE_MODE}, SAFETY_LOOKBACK_DAYS={SAFETY_LOOKBACK_DAYS}, MAX_DAYS_PER_RUN={MAX_DAYS_PER_RUN}")
    print("=" * 120)

    bootstrap_fct_database(engine)

    d_from, d_to = resolve_date_range(engine)
    print(Demon_f"[RUN] DATE_RANGE : {d_from} ~ {d_to}")

    cur = d_from
    while cur <= d_to:
        day_start = time.time()
        print("\n" + "-" * 120)
        print(Demon_f"[RUN] DAY : {cur}")
        print("-" * 120)
        ...
        print(Demon_f"[RUN] DAY DONE : {cur} / rows_final={len(df_final)} / sec={time.time()-day_start:.2f}")
    ...
    print("=" * 120)
    print(Demon_f"[RUN] END   : {run_end.strftime('%Y-%m-%d %H:%M:%S')}")
    print(Demon_f"[RUN] SEC   : {run_seconds:.2f} sec")
    print("=" * 120)
