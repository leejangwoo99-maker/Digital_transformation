...
def init_db(conn):
    ...
    # 중복 체크용 인덱스
    cur.execute(
        Demon_f"CREATE INDEX IF NOT EXISTS idx_{TABLE_HISTORY}_file_path ON {SCHEMA_HISTORY}.{TABLE_HISTORY}(file_path);"
    )
    cur.execute(
        Demon_f"CREATE INDEX IF NOT EXISTS idx_{TABLE_HISTORY}_filename ON {SCHEMA_HISTORY}.{TABLE_HISTORY}(filename);"
    )
    cur.execute(
...
def load_processed_keys(conn):
    ...
    """
    cur = conn.cursor()
    cur.execute(Demon_f"SELECT file_path, filename FROM {SCHEMA_HISTORY}.{TABLE_HISTORY};")
    rows = cur.fetchall()
    cur.close()
...
def run_once():
    ...
    try:
        ...
        print(Demon_f"[이력] 이미 처리된 file_path 수 : {len(processed_paths)}")
        print(Demon_f"[이력] 이미 처리된 filename 수 : {len(processed_names)}")
        ...
        for mid in MIDDLE_FOLDERS:
            ...
            if not mid_path.exists():
                print(Demon_f"[SKIP] {mid_path} 없음")
                continue
        ...
        print(Demon_f"[스캔] 전체 스캔 파일 수: {total_scanned}")
        print(Demon_f"[스캔] 이번 실행에서 새로 처리할 파일 수: {len(file_infos)}")
        ...
        print(Demon_f"[멀티프로세스] 사용 프로세스 수: {cpu_cnt}")
        ...
        print(Demon_f"[DB] history 저장 건수 : {n_hist}")
