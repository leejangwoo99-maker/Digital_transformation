...
def _reset_connection(reason: str = ""):
    ...
    try:
        if reason:
            log_print(Demon_f"[DB][RESET] {reason}", info="down")
    except Exception:
        pass
    try:
        _safe_close(_CONN)
...
def get_connection_blocking():
    while True:
        try:
            ...
            log_print(
                Demon_f"[DB][OK] connected (work_mem={WORK_MEM}, "
                Demon_f"keepalive={PG_KEEPALIVES}/{PG_KEEPALIVES_IDLE}/{PG_KEEPALIVES_INTERVAL}/{PG_KEEPALIVES_COUNT})",
                info="info",
            ...
            msg = Demon_f"[DB][RETRY] connect failed: {type(e).__name__}: {repr(e)}"
...
def ensure_connection():
    ...
    try:
        ...
        if _is_connection_error(e):
            log_print(Demon_f"[DB][WARN] connection unhealthy, will reconnect: {type(e).__name__}: {repr(e)}", info="down", save_db=False)
...
def ensure_health_log_table(conn):
    ...
    cur = conn.cursor()
    try:
        cur.execute(Demon_f"CREATE SCHEMA IF NOT EXISTS {SCHEMA_HEALTH};")
        cur.execute(
            Demon_f"""
...
def hold_console(exit_code: int) -> None:
    try:
        log_print("\n" + "=" * 70, info="info")
        log_print(Demon_f"[HOLD] exit_code={exit_code}", info="info")
        log_print(Demon_f"[HOLD] 로그 파일: {LOG_FILE}", info="info")
...
def init_db(conn):
    ...
    # 기존 요구사항 유지
    cur.execute(Demon_f"DROP SCHEMA IF EXISTS {SCHEMA_RESULT} CASCADE;")
    cur.execute(Demon_f"DROP SCHEMA IF EXISTS {SCHEMA_DETAIL} CASCADE;")

    cur.execute(Demon_f"CREATE SCHEMA IF NOT EXISTS {SCHEMA_HISTORY};")
    cur.execute(
        Demon_f"""
    ...
    cur.execute(
        Demon_f"CREATE INDEX IF NOT EXISTS idx_{TABLE_HISTORY}_barcode ON {SCHEMA_HISTORY}.{TABLE_HISTORY}(barcode_information);"
    )
...
def run_once():
    ...
    log_print(Demon_f"시각: {started_at} / end_day(today)={today_yyyymmdd} / window={REALTIME_WINDOW_SEC}s", info="info")
    ...
    while True:
        ...
        try:
            ...
            log_print(Demon_f"[이력] 오늘(end_day={today_yyyymmdd}) 처리된 file_path 수 : {len(processed_paths)}", info="info")
            ...
            for mid in MIDDLE_FOLDERS:
                ...
                if not mid_path.exists():
                    log_print(Demon_f"[SKIP] {mid_path} 없음", info="warn")
                    continue
            ...
            log_print(Demon_f"[스캔] 전체 스캔 파일 수(오늘 폴더 내): {total_scanned}", info="info")
            log_print(Demon_f"[스캔] 이번 실행에서 새로 처리할 파일 수(120초 이내): {len(file_infos)}", info="info")

            if not file_infos:
                log_print("[정보] 새로 처리할 파일이 없습니다.", info="sleep")
                return

            log_print(Demon_f"[멀티프로세스] 사용 프로세스 수: {MP_PROCESSES}", info="info")
            ...
            log_print(Demon_f"[DB] history INSERT 시도 건수 : {n_try} (중복은 DB에서 자동 무시)", info="info")

            return

        except psycopg2.Error as e:
            if _is_connection_error(e):
                log_print(Demon_f"[DB][RETRY] connection lost during operation: {type(e).__name__}: {repr(e)}", info="down", save_db=False)
                try:
                    conn.rollback()
                except Exception:
                    pass
            ...
            log_print(Demon_f"[DB][FATAL] non-connection db error: {type(e).__name__}: {repr(e)}", info="error")
            raise
...
if __name__ == "__main__":
    ...
    try:
        while True:
            run_once()
            log_print(Demon_f"[SLEEP] {LOOP_INTERVAL_SEC}초 대기", info="sleep")
